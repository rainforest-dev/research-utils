...
parser = argparse.ArgumentParser(
    prog='ImgGenerator', description='Generate images with different pattern')
...
parser.add_argument(
    ...
    default='f',
    help=
    'available styles: f (fibonacci), p (poisson disc), h (hexagon), s (square), r (random)'
)
...
if style == 'r':
    """Random"""
    points = np.concatenate((np.random.uniform(
        0, lx, size=(num, 1)), np.random.uniform(0, ly, size=(num, 1))),
                            axis=1)
    ...
    for i in range(nx):
        for j in range(ny):
            points[id_, :] = np.dot(np.transpose(unit),
                                    [[i - mx], [j - my]]).reshape(-1)
            id_ = id_ + 1
    points[:, 0] = points[:, 0] - (
        (np.max(points[:, 0]) + np.min(points[:, 0])) / 2.0 -
        lx / 2.0) - np.dot(np.transpose(unit), [[0.5], [0.5]])[0]
    points[:, 1] = points[:, 1] - (
        (np.max(points[:, 1]) + np.min(points[:, 1])) / 2.0 -
        ly / 2.0) - np.dot(np.transpose(unit), [[0.5], [0.5]])[1]
    ...
    """Hexagon"""
    s = (lx + ly) / (2 * math.sqrt(num))
    unit = s * np.array([[1, 0], [0, math.sqrt(3.)]],
                        dtype=float)  # distant unit
    ...
    for i in range(nx):
        for j in range(ny):
            ...
            points[id_, :] = np.dot(
                np.transpose(unit),
                [[i + 0.5 - mx], [j + 0.5 - my]]).reshape(-1)
            id_ = id_ + 1
    points[:, 0] = points[:, 0] - (
        (np.max(points[:, 0]) + np.min(points[:, 0])) / 2.0 -
        lx / 2.0) - np.dot(np.transpose(unit), [[0.25], [0.25]])[0]
    points[:, 1] = points[:, 1] - (
        (np.max(points[:, 1]) + np.min(points[:, 1])) / 2.0 -
        ly / 2.0) - np.dot(np.transpose(unit), [[0.25], [0.25]])[1]
    ...
    def fibonacci(n):
        ...
        for i in range(n):
            if point[i, 0] <= lx and point[i, 0] >= 0 and point[
                    i, 1] <= ly and point[i, 1] >= 0:
                count = count + 1
        return point, count
...
if __name__ == "__main__":
    points = np.append(points,
                       [[1000 * lx, 1000 * ly], [-999 * lx, 1000 * ly],
                        [1000 * lx, -999 * ly], [-999 * lx, -999 * ly]],
                       axis=0)
    ...
    if args.outfile is None:
        i = 0
        while True:
            outfile = '{:s}_{:3d}_{:.1f}_{:02d}.png'.format(
                args.style.upper(), args.num, args.porosity, i)
    ...
    while True:
        ...
        if abs(
            (porosity - args.porosity) / args.porosity) < 1e-2 or iter >= 50:
            break
        else:
            if iter == 0:
                descent = ((porosity - args.porosity) / args.porosity)
            else:
                descent = 0.5 * descent + 0.5 * (
                    (porosity - args.porosity) / args.porosity)

            width = width + descent * rate * width

            iter = iter + 1
            print('Iter {} \t Porosity = {:f} \t Try width {:f}'.format(
                iter, porosity, width),
                  end='\r')

            plt.close()

    print('Iter {} \t Porosity = {:f} \t Final wall width {:f}'.format(
        iter, porosity, width))
